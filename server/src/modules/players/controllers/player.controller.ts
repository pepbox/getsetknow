...
export const onboardPlayer = async (
  ...
  try {
    const { name, session, teamNumber } = req.body;
...
export const submitGuess = async (
  ...
  try {
    ...
    let profilePicture = "";
    let score = 0;
    const guessedPerson = await playerService.getPlayerById(
...
export const getPlayerStats = async (
  ...
  try {
    ...
    // People who know you
    const guessesByPerson = await playerService.getGuessesByPersonId(
      player._id
    );
...
export const getGameCompletionData = async (
  ...
  try {
    ...
    if (currentPlayer.profilePhoto) {
      const file = await fileService.getFileById(
        currentPlayer.profilePhoto.toString()
      );
      currentPlayerProfilePhoto = file?.location || "";
    }

    // Get all players in session
    let allPlayers = await playerService.getPlayersBySession(
      new Types.ObjectId(sessionId)
    );
    ...
    const guessesByUser = await playerService.getGuessesByUserId(
